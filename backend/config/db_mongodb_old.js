...
// create our function
export async function connectToDatabaseOld() {
    ...
    try {
        ...
        // comment out process.exit( 1 ); since I do not want to stop the server if there
        // is an error
        // exit with an uncaught fatal exception
        // process.exit( 1 );

        // if we can not connect to the database then set " client " equal to
        // " databaseConnectionError " and then we can test for this inside our API route
        // and if this evaluates to true then we know there was a database connection error
        // and we can then set an error message inside the API route and send that error
        // message to the frontend
        let client = 'databaseConnectionError';
