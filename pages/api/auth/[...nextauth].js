...
// ==============================
// this file was built using mongodb and not mongoose, if we want to use mongoose then we
// need to change:
// ==============================
// 1 - add this file to the api/old-files folder and change the name to
// [...nextauth]_mongodb.js
// 2 - take the [...nextauth]_mongoose.js file in the api/old-files folder and change the
// name to [...nextauth].js and insert the file into the api/auth folder
// 3 - remember to keep only one file in the api/auth folder at a time and the file must be
// named [...nextauth].js
...
import { hashedPassword, verifyPassword } from '../../../backend/utils/password_utils';

// when we call NextAuth(); then next auth will create and return a handler function
// and we need this handler function because this is still an api route and when we call
// NextAuth we can pass in a configuration object and this object will allow us to configure
// NextAuth's behavior

// in our case we want to set the providers option and set it equal to an array

// we will use the Credentials provider and this means that we will use our own
// credentials for authentication

// from the next auth docs: " The Credentials provider allows you to handle signing in
// with arbitrary credentials, such as a username and password, domain, or two factor
// authentication or hardware device (e.g. YubiKey U2F / FIDO). "

// the Credentials provider takes in a configuration object and inside this configuration
// object we can do a number of things but the only thing we need to do here is set
// authorize and authorize is a method and next.js will call this method for us when it
// receives an incoming sign in request

// and authorize is an async function so it returns a promise and as an argument we will
// pass in the credentials object and this object represents the data that was sent along
// with the user sign in request or ( see the signin.js file for details ):
...
export default NextAuth(
    ...
    { // configuration object
        ...
        // besides setting up our providers ( see below ) let's add the session object below

        // the session object will allow us to config how the session for the authenticated
        // user will be managed and set the jwt key to true so that we indicate to next auth
        // that json web tokens are being used
            ...
            CredentialsProvider(
                ...
                { // configuration object
                    ...
                    async authorize( credentials ) {
                        ...
                        // inside this function we have bring in our own authorization logic
                        // and in the process check and see if the credentials are valid
                        // and next we want to connect to the database
                        ...
                        // get the const client
                        const client = await connectToDatabase();

                        // access to the database by doing " client.db(); " and then save the
                        // result to the const " db "
                        const db = client.db();
                        ...
                        // access the users collection and save the result to the const
                        // " usersCollection "
                        ...
                        // retrieve the corresponding user document and save that user document to
                        // the const " user "
                        ...
                        if ( !user ) {
                            ...
                            // close the database connection and call client.close(); before
                            // we throw an Error
                            client.close();
                        ...
                        const passwordsAreEqual = await verifyPassword( credentials.password, databasePassword )

                        // if the databasePassword matches the entered password then proceed
                        // forward
                        if ( passwordsAreEqual ) {
                            ...
                            // with next auth the object below will be encoded into the JWT
                            return {
                                name  : user.name,
                                email : user.email
                            };
